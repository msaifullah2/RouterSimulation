...
def Router_6():
	...
	def user_inputs():
		...
		inputfile1 = 'Router_5_Port_2'
		inputfile2 = 'Router_5_Port_3'
	...
	def ouput_forward(srcaddr, destaddr, data, outputportSpeed001, outputportSpeed002, outputportSpeed003):
			...
			check_ip.append(ipaddr.IPNetwork(str('200.177.144.0/20')))   
			check_ip.append(ipaddr.IPNetwork(str('212.18.32.0/19'))) 
			check_ip.append(ipaddr.IPNetwork(str('212.18.44.0/22'))) 
			...
			if dest_Port_queue[0] == '1':
				...
				if (dest_Port_queue[1] == '1'):
					ran_data = q2a.pop()
					write_Output('Router_6_Port_2', ran_data)
					write_Output('R106', ran_data)
					stop2a = timeit.default_timer()
					list2a.append(stop2a-start2a)
				elif (dest_Port_queue[1] == '2'): 
					write_Output('Router_6_Port_2', q2b.pop())
					stop2b = timeit.default_timer()
					list2b.append(stop2b-start2b)
				else:
					if check_ip[0].Contains(checkaddress):  
						write_Output('Destination3', q2c.pop()) 
						Destination3_count = Destination3_count + 1 
					else: 
						write_Output('Router_6_Port_2', q2c.pop())
				...
				if (dest_Port_queue[1] == '1'):
					write_Output('Router_6_Port_3', q3a.pop())
					stop3a = timeit.default_timer()
					list3a.append(stop3a-start3a)
				elif (dest_Port_queue[1] == '2'):
					if check_ip[1].Contains(checkaddress):  
						write_Output('Destination3', q3b.pop()) 
						Destination3_count = Destination3_count + 1 
					else: 
						write_Output('Router_6_Port_3', q3b.pop())
						stop3b = timeit.default_timer()
						list3b.append(stop3b-start3b)
				else:
					if check_ip[2].Contains(checkaddress):  
						write_Output('Destination3', q3c.pop()) 
						Destination3_count = Destination3_count + 1 
					else:
						write_Output('Router_6_Port_3', q3c.pop())
	...
	if queueSpeed002_a != 0 and queueSpeed002_b != 0 and queueSpeed002_c != 0:
	   if queue_2_a_cnt != 0: 
			utilization_queue_2_a = float(chunksize_b / queue_2_a_cnt) / (queueSpeed002_a * 1000)
	   ...
	   if queue_2_b_cnt != 0: 
		   utilization_queue_2_b = float(chunksize_b / queue_2_b_cnt) / (queueSpeed002_b * 1000)
	   ...
	   if queue_2_c_cnt != 0: 
		   utilization_queue_2_c = float(chunksize_b / queue_2_c_cnt) / (queueSpeed002_c * 1000)
	...
	if queueSpeed003_a != 0 and queueSpeed003_b != 0 and queueSpeed003_c != 0:
	   if queue_3_a_cnt != 0: 
			utilization_queue_3_a = float(chunksize_c / queue_3_a_cnt) / (queueSpeed003_a * 1000)
	   ...
	   if queue_3_b_cnt != 0: 
		   utilization_queue_3_b = float(chunksize_c / queue_3_b_cnt) / (queueSpeed003_b * 1000)
	   ...
	   if queue_3_c_cnt != 0: 
		   utilization_queue_3_c = float(chunksize_c / queue_3_c_cnt) / (queueSpeed003_c * 1000)
	...
	print '\n\n\n'
	print '-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*Router 6 Analysis Report-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*'
	print '\n\n\n'
	...
	print '\n'
	print 'Packets forwarded to next-hop R106 : ', r106count
	print '\n'
	'''
	...
	print '\n'
	print "Executed Router 6 succesfully!"
