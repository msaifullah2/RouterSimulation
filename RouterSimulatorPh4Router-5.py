...
def Router_5():
	...
	def user_inputs():
		...
		inputfile2 = 'Router_1_Port_3'
		inputfile3 = 'Router_3_Port_2'
	...
	def ouput_forward(srcaddr, destaddr, data, outputportSpeed001, outputportSpeed002, outputportSpeed003):
			...
			check_ip.append(ipaddr.IPNetwork(str('216.15.64.0/18')))   
			check_ip.append(ipaddr.IPNetwork(str('220.210.0.0/16'))) 
			try:
			...
			if dest_Port_queue[0] == '1':
				...
				if (dest_Port_queue[1] == '1'):  
					write_Output('Router_5_Port_2', q2a.pop())
					stop2a = timeit.default_timer()
					list2a.append(stop2a-start2a)
				elif (dest_Port_queue[1] == '2'):  
					if check_ip[0].Contains(checkaddress):  
						write_Output('R105', q2b.pop()) 
						r105count = r105count + 1 
					else:
						write_Output('Router_5_Port_2', q2b.pop())
						stop2b = timeit.default_timer()
						list2b.append(stop2b-start2b)
				else: 
					write_Output('Router_5_Port_2', q2c.pop())
				...
				if (dest_Port_queue[1] == '1'):
					if check_ip[1].Contains(checkaddress):  
						write_Output('R105', q3a.pop()) 
						r105count = r105count + 1 
					else:
						write_Output('Router_5_Port_3', q3a.pop())
						stop3a = timeit.default_timer()
						list3a.append(stop3a-start3a)
				elif (dest_Port_queue[1] == '2'):
					write_Output('Router_5_Port_3', q3b.pop())
					stop3b = timeit.default_timer()
					list3b.append(stop3b-start3b)
				else:
					write_Output('Router_5_Port_3', q3c.pop())
	...
	print '\n\n\n'
	print '-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*Router 5 Analysis Report-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*-*'
	print '\n\n\n'
	...
	print '\n'
	print 'Packets forwarded to next-hop R105 : ', r105count
	print '\n'
	'''
	...
	print '\n'
	print "Executed Router 5 succesfully!"
